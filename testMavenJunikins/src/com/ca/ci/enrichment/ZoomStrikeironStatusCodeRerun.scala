  ...
  */
  ...
  def main(arg: Array[String]) = {
    ...
    */
    ...
    val instanceName="Zoom_Strikeiron_Status_CodeRerun_load_"+System.currentTimeMillis()
  ...
  def processRecords(sparkSession: SparkSession,ruleMap:scala.collection.mutable.Map[String, String],jdbcURL:String,connectionProperties:Properties): Unit = {
    ...
    try {
      ...
      */
      ...
      val strikeironEnrichmentDF = sparkSession.read.jdbc(jdbcURL, "mdm.enrichment_api_person_email_verification_temp", connectionProperties).where("statusNbr='220' and Code='222'")
      strikeironEnrichmentDF.createOrReplaceTempView("ZoomStrikeironStatusCodeRerunTempView")
      /*
      filter the distinct email address
      */
      val filteredEmaiDF=sparkSession.sql("select distinct Email from ZoomStrikeironStatusCodeRerunTempView")
      ...
      */
      ...
      {
        case e: Exception => {
          print("ERROR:Exception in processRecords"+e.toString())
          e.printStackTrace()
        }
      }finally
    {
      ...
      */
      ...
      if(!emailList.isEmpty) {
        emailsrecordList.createOrReplaceTempView("emailsRecordListPersonEmailRerun")
        val emailDF = sparkSession.sql("select distinct a.*,b.person_key,b.person_email_key,current_timestamp() as email_verification_date from emailsRecordListPersonEmailRerun a,ZoomStrikeironStatusCodeRerunTempView b where a.email_address=b.Email")
        val dropEmailDF = emailDF.drop("email_address")
        dropEmailDF.write.mode(SaveMode.Append).jdbc(jdbcURL, "mdm.enrichment_api_person_email_verification_temp_rerun", connectionProperties)
      }
    }
  }

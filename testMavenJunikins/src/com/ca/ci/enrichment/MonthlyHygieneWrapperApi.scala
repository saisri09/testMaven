...
/**
  ...
  */
object MonthlyHygieneWrapperApi {
  /*
  ...
  sparkjob constant
   */
  val appname = "MonthlyHygieneWrapperApi"
  val file_key = 1
  val started = "started"
  val ended = "ended"
  ...
  /*val sparkConf = new SparkConf().setAppName(appname).setMaster("local")
    val sparkContext = new SparkContext(sparkConf)
    val sparkSession = SparkSession.builder.appName("MonthlyHygieneWrapperApi").config("spark.sql.warehouse.dir","file:///C:/tempwarehouse").getOrCreate()
    */
   ...
   // print("entity_detail_key: "+entity_detail_key)
    ...
    val file_key=1
    val instanceName="Monthly_Hygiene_WrapperApi_load"+System.currentTimeMillis()
  ...
  def processRecords(sparkSession: SparkSession,ruleMap:scala.collection.mutable.Map[String, String]): Unit = {
   ...
   val dbPersonPhoneAndPerson=dbBatchRunDetails.join(dbEnrichmentApiPersonPhone,"person_key")
    ...
    val dbPersonRunfilter = sparkSession.sql("select * from dbPersonPhoneAndPersonTempView where updated_date like'" + currentMonth + "%'")
